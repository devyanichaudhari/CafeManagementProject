...
export class RecipeService {
  ...
  getRecipes(): Observable<any> {
    ...
    return this.http.get<any>(`${this.apiUrl}/api/recipes`, { headers }).pipe(
      tap((recipes) => {
  ...
  createRecipe(recipe: any): Observable<any> {
    const headers = this.getHeaders();
    return this.http
      .post<any>(`${this.apiUrl}/api/recipes`, recipe, { headers })
      .pipe(
        tap((newRecipe) => {
          const currentState = this.recipesSubject.value;
  ...
  updateRecipe(recipe: any): Observable<any> {
    const headers = this.getHeaders();
    return this.http
      .put<any>(`${this.apiUrl}/api/recipes/${recipe.id}`, recipe, { headers })
      .pipe(
        tap((updatedRecipe) => {
          const currentState = this.recipesSubject.value;
  ...
  likeRecipe(recipeId: number): Observable<any> {
    const headers = this.getHeaders();
    return this.http
      .put<any>(`${this.apiUrl}/api/recipes/${recipeId}/like`, {}, { headers })
      .pipe(
        tap((updatedRecipe) => {
          const currentState = this.recipesSubject.value;
          const updatedRecipes = currentState.recipes.map((item: any) =>
  ...
  deleteRecipe(id: any): Observable<any> {
    const headers = this.getHeaders();
    return this.http.delete<any>(`${this.apiUrl}/api/recipes/${id}`, {
      headers,
    }).pipe(
      tap((deletedRecipe)=>{
        const currentState=this.recipesSubject.value;
